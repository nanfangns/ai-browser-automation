...
export class PromptHandler {
    ...
    /**
     * Attempts to get a cached snapshot if available, without forcing a fresh CDP call.
     * Falls back to null if no cached snapshot exists.
     */
    async _getCachedSnapshotIfFresh() {
        if (!this.controlManager) return null;
        // The snapshotManager doesn't expose a cache, so we rely on the last taken snapshot.
        // We return null to trigger a fresh take which is safer for state-changing tools.
        // This method is a placeholder for future optimization where we cache the last snapshot.
        return null;
    }
                ...
                while (keepLooping && loopCount < MAX_LOOPS) {
                    ...
                    if (toolResult) {
                        ...
                        const skipSnapshotTools = [
                            ...
                            'list_pages',
                            'batch'  // Batch execution already handles snapshot refresh internally
                        ];
                        ...
                        if (request.enableBrowserControl && this.controlManager && !skipSnapshotTools.includes(toolResult.toolName)) {
                             try {
                                 ...
                                 // Only refresh snapshot if the tool changed state OR if stale UID recovery happened
                                 const shouldRefresh = toolResult.needsSnapshotRefresh ||
                                     (toolResult.toolName === 'take_snapshot');
                                 const snapshot = shouldRefresh
                                     ? await this.controlManager.getSnapshot()
                                     : await this._getCachedSnapshotIfFresh();
