...
export function requestCustomPromptFromStorage() {
    window.parent.postMessage({ action: 'GET_CUSTOM_PROMPT' }, '*');
}

export function saveCustomPromptToStorage(prompt) {
    window.parent.postMessage({
        action: 'SAVE_CUSTOM_PROMPT',
        payload: prompt
    }, '*');
}